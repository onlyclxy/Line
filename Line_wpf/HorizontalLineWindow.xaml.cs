...
{
    // 可拖拽的横线类
    public class DraggableHorizontalLine : Window
    {
        ...
        public DraggableHorizontalLine(int width, int height, System.Drawing.Color color, double opacity, bool clickThrough)
        {
            ...
            if (!mouseClickThrough)
            {
                ...
                this.Cursor = System.Windows.Input.Cursors.SizeNS; // 设置为垂直调整大小光标
            }
        }
    ...
    public partial class HorizontalLineWindow : Window
    {
        ...
        private MainWindow mainWindow; // 添加对主窗体的引用

        // 线条默认高度为1像素
        private int lineHeight = 1;

        // 线条颜色，默认为绿色
        private System.Drawing.Color lineColor = System.Drawing.Color.Green;
        ...
        public HorizontalLineWindow(NotifyIcon existingTrayIcon, MainWindow mainWindow)
        {
        ...
        {
            Console.WriteLine("[横线] 开始预创建横线窗体");
            
            // 为每个可能的热键索引预创建线条
            for (int i = 0; i < 4; i++)
            {
                var lines = new List<DraggableHorizontalLine>();
                ...
                if (showOnAllScreens)
                {
                    ...
                    {
                        var line = CreateHorizontalLine(screen.Bounds.Width);
                        line.SetPhysicalBounds(screen.Bounds.X, 0, screen.Bounds.Width, lineHeight);
                        line.Opacity = 0; // 初始隐藏
                        line.Show(); // 显示但透明
                        lines.Add(line);
                        Console.WriteLine($"[横线] 为热键 {i + 1} 预创建屏幕线条: 屏幕{screen.Bounds}");
                    }
                }
                else
                {
                    ...
                    var line = CreateHorizontalLine(primaryScreen.Bounds.Width);
                    line.SetPhysicalBounds(primaryScreen.Bounds.X, 0, primaryScreen.Bounds.Width, lineHeight);
                    line.Opacity = 0; // 初始隐藏
                    line.Show(); // 显示但透明
                    lines.Add(line);
                    Console.WriteLine($"[横线] 为热键 {i + 1} 预创建单屏线条: 主屏幕{primaryScreen.Bounds}");
                }
                
                horizontalLines[i] = lines;
                lineStates[i] = false;
            }
            
            Console.WriteLine("[横线] 横线窗体预创建完成");
        }

        /// <summary>
        /// 创建一个横线窗体
        /// </summary>
        private DraggableHorizontalLine CreateHorizontalLine(int width)
        {
            var line = new DraggableHorizontalLine(
                width,
                lineHeight,
                lineColor,
                lineOpacity / 100.0,
        ...
        private void RegisterHotkeyPair(int index)
        {
            try
            {
                if (index >= 0 && index < 4)
                {
                    ...
                    Console.WriteLine($"[横线] RegisterHotkeyPair: 窗口句柄: {hwnd}");
                    
                    if (hwnd != IntPtr.Zero)
                    {
                        // 注册 Ctrl+1-4
                        bool onSuccess = RegisterHotKey(hwnd, BASE_HOTKEY_ID_ON + index, MOD_CONTROL, (int)key);
                        Console.WriteLine($"[横线] RegisterHotKey ON (Ctrl+{index + 1}): {onSuccess}");
                        
                        // 注册 Ctrl+Shift+1-4
                        bool offSuccess = RegisterHotKey(hwnd, BASE_HOTKEY_ID_OFF + index, MOD_CONTROL | MOD_SHIFT, (int)key);
                        Console.WriteLine($"[横线] RegisterHotKey OFF (Ctrl+Shift+{index + 1}): {offSuccess}");

                        if (onSuccess && offSuccess)
                        {
                            hotkeyEnabled[index] = true;
                            Console.WriteLine($"[横线] 热键 {index + 1} 注册成功");
                        }
                        else
                        {
                            // 如果注册失败，注销已注册的热键
                            if (onSuccess) {
                                ...
                                Console.WriteLine($"[横线] 回滚已注册的ON热键 {index + 1}");
                            }
                            if (offSuccess) {
                                UnregisterHotKey(hwnd, BASE_HOTKEY_ID_OFF + index);
                                Console.WriteLine($"[横线] 回滚已注册的OFF热键 {index + 1}");
                            }
                            
                            hotkeyEnabled[index] = false;
                            Console.WriteLine($"[横线] 热键 {index + 1} 注册失败");
                            System.Windows.MessageBox.Show($"横线热键 Ctrl+{index + 1} 注册失败，可能已被其他程序占用。", "警告", MessageBoxButton.OK, MessageBoxImage.Warning);
                        }
                    }
                    else
                    {
                        Console.WriteLine($"[横线] 窗口句柄为空，无法注册热键 {index + 1}");
                        hotkeyEnabled[index] = false;
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                SaveConfig();
                System.Windows.MessageBox.Show($"注册热键时发生错误: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        {
            Console.WriteLine($"[横线] ToggleHotkeyBinding called: index={index}");
            
            if (index >= 0 && index < 4)
            {
                bool newState = !hotkeyEnabled[index];
                Console.WriteLine($"[横线] 当前状态: {hotkeyEnabled[index]} -> 目标状态: {newState}");
                ...
                if (newState)
                {
                    ...
                    if (mainWindow?.IsGlobalHotkeysDisabled == true)
                    {
                        Console.WriteLine($"[横线] 全局快捷键已被禁用，无法注册热键 {index + 1}");
                        System.Windows.MessageBox.Show("全局快捷键已被禁用，无法注册横线热键。", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                        return;
                    }
                    ...
                    var hwnd = new WindowInteropHelper(this).Handle;
                    Console.WriteLine($"[横线] 尝试注册热键 {index + 1}: Ctrl+{index + 1} 和 Ctrl+Shift+{index + 1}");
                    Console.WriteLine($"[横线] 窗口句柄: {hwnd}");
                    ...
                    else
                    {
                        Console.WriteLine($"[横线] 窗口句柄为空，无法注册热键 {index + 1}");
                        hotkeyEnabled[index] = false;
                        System.Windows.MessageBox.Show($"横线热键注册失败：窗口句柄无效。", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
                else
                {
                    Console.WriteLine($"[横线] 注销热键 {index + 1}");
                    // 注销热键
                    ...
                    if (horizontalLines.ContainsKey(index))
                    {
                        ...
                        Console.WriteLine($"[横线] 移除了现有的横线 {index + 1}");
                    }
                }
                
                Console.WriteLine($"[横线] 更新菜单状态和保存配置");
                UpdateHotkeyMenuCheckedState();
                SaveConfig();
                Console.WriteLine($"[横线] ToggleHotkeyBinding 完成: index={index}, 最终状态={hotkeyEnabled[index]}");
            }
        }

        private void UpdateHotkeyMenuCheckedState()
        {
            Console.WriteLine($"[横线] UpdateHotkeyMenuCheckedState called");
            if (trayIcon?.ContextMenuStrip == null) 
            {
                Console.WriteLine($"[横线] trayIcon?.ContextMenuStrip 为空");
                return;
            }
                ...
                if (item is ToolStripMenuItem menuItem && menuItem.Text == "持续横线")
                {
                    ...
                    foreach (ToolStripItem subItem in menuItem.DropDownItems)
                    {
                        if (subItem is ToolStripMenuItem subMenuItem && subMenuItem.Text == "热键绑定")
                        {
                            ...
                            Console.WriteLine($"[横线] 找到热键绑定子菜单");
                            break;
                        }
                    }
                    ...
                    if (hotkeyMenu != null)
                    {
                        Console.WriteLine($"[横线] 开始更新热键菜单状态, 热键数量: {hotkeyMenu.DropDownItems.Count}");
                        // 更新热键绑定菜单项的选中状态
                        for (int i = 0; i < Math.Min(hotkeyMenu.DropDownItems.Count, hotkeyEnabled.Length); i++)
                        {
                            if (hotkeyMenu.DropDownItems[i] is ToolStripMenuItem subItem)
                            {
                                ...
                                Console.WriteLine($"[横线] 热键 {i + 1}: {oldChecked} -> {hotkeyEnabled[i]}");
                            }
                        }
                    }
                    else
                    {
                        Console.WriteLine($"[横线] 未找到热键绑定子菜单");
                    }
                    break;
                }
            }
        ...
        private void ShowHorizontalLine(int index)
        {
            ...
            Console.WriteLine($"[横线] ShowHorizontalLine({index}) 开始");
            Console.WriteLine($"[横线] 鼠标位置: ({mousePos.X}, {mousePos.Y})");
            Console.WriteLine($"[横线] 显示模式: {(showOnAllScreens ? "所有屏幕" : "单屏幕")}");
            
            // 检查是否有预创建的线条
            if (!horizontalLines.ContainsKey(index))
            {
                Console.WriteLine($"[横线] 错误：没有找到预创建的横线 {index}");
                return;
            }

            var lines = horizontalLines[index];
            Console.WriteLine($"[横线] 找到预创建的横线 {index}，数量: {lines.Count}");
            
            if (showOnAllScreens)
            {
                // 在所有屏幕显示横线
                var mouseScreen = Screen.FromPoint(mousePos);
                Console.WriteLine($"[横线] 鼠标所在屏幕: {mouseScreen.Bounds}");
                int relativeY = mousePos.Y - mouseScreen.Bounds.Y;
                Console.WriteLine($"[横线] 相对Y坐标: {relativeY}");
                ...
                foreach (Screen screen in Screen.AllScreens)
                {
                    if (lineIndex < lines.Count)
                    {
                        ...
                        Console.WriteLine($"[横线] 更新屏幕 {screen.Bounds} 线条位置: Y={lineY}");
                        line.SetPhysicalBounds(screen.Bounds.X, lineY, screen.Bounds.Width, lineHeight);
                        line.Opacity = lineOpacity / 100.0;
            ...
            else
            {
                ...
                Console.WriteLine($"[横线] 单屏模式 - 鼠标所在屏幕: {currentScreen.Bounds}");
                
                if (lines.Count > 0)
                {
                    var line = lines[0]; // 单屏模式只使用第一条线
                    Console.WriteLine($"[横线] 更新横线位置: ({currentScreen.Bounds.X}, {mousePos.Y})");
                    line.SetPhysicalBounds(currentScreen.Bounds.X, mousePos.Y, currentScreen.Bounds.Width, lineHeight);
                    line.Opacity = lineOpacity / 100.0;
            ...
            Console.WriteLine($"[横线] ShowHorizontalLine({index}) 完成");
        }

        private void HideHorizontalLine(int index)
        {
            Console.WriteLine($"[横线] HideHorizontalLine({index}) 开始");
            if (horizontalLines.ContainsKey(index))
            {
                Console.WriteLine($"[横线] 隐藏横线 {index}，数量: {horizontalLines[index].Count}");
                foreach (var line in horizontalLines[index])
                {
                    line.Opacity = 0; // 只隐藏，不关闭
                }
                lineStates[index] = false;
                Console.WriteLine($"[横线] HideHorizontalLine({index}) 完成");
            }
            else
            {
                Console.WriteLine($"[横线] 警告：没有找到要隐藏的横线 {index}");
            }
        }
        ...
        private void PerformHotkeyRegistration()
        {
            ...
            Console.WriteLine($"[横线] 热键注册结果: {successCount}/{totalEnabled} 成功");
            
            // 如果部分失败，可以选择重试
            if (successCount < totalEnabled && successCount > 0)
            {
                Console.WriteLine("[横线] 部分热键注册失败，但有部分成功，继续运行");
            }
        }
        ...
        private void SaveConfig()
        {
            ...
            {
                System.Windows.MessageBox.Show($"保存横线配置时出错: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
    }
} 